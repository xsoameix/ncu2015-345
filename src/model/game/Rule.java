...
public class Rule {
	...
	public MapBlock getCurrentMapBlock(FieldObject objects){
		return map.getMapBlock(objects.getLocation().x/MapBlock.getSize().width,
						       objects.getLocation().y/MapBlock.getSize().height);
	}
		...
		if(collusionObject == null || (collusionObject instanceof Turf))
		{
			...
			for(int row = currentPosY; row <= currentPosY+1; row++){
				for(int col = currentPosX; col <= currentPosY+1; col++){
					...
					rect = new Rectangle(col*MapBlock.getSize().width,
							             row*MapBlock.getSize().height,
